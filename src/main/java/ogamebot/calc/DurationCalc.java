...
public class DurationCalc {
    ...
    public static Duration calculateDuration(UnitType<?> type, int counter, CelestialBody body) {
        if (type instanceof BuildingType) {
            ...
            if (body instanceof Planet) {
                Building machFactory = body.getBuilding(BuildingType.ROBOFACTORY);
                Building nani = body.getBuilding(BuildingType.NANOFACTORY);

                machLvl = machFactory.getCounter();
                naniLevel = nani.getCounter();
            } else if (body instanceof Moon) {
                machLvl = body.getBuilding(BuildingType.ROBOFACTORY).getCounter();
            }
            ...
            if (body instanceof Moon) {
                final Building shipYard = body.getBuilding(BuildingType.SPACESHIPSHIPYARD);
                shipYardLvl = shipYard.getCounter();
            } else {
                final Building shipYard = body.getBuilding(BuildingType.SPACESHIPSHIPYARD);
                final Building nanoFactory = body.getBuilding(BuildingType.NANOFACTORY);
            ...
            final Building researchLab = body.getBuilding(BuildingType.RESEARCHLAB);
    ...
    public Duration calc() {
        if (type instanceof BuildingType) {
            ...
            final int level = body.getBuilding((BuildingType) type).getCounter();
